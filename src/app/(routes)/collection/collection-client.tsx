...
export function CollectionClient() {
  ...
  async function assignSlot(page: number, position?: number | null) {
    if (!activeBinder || !binderCardSelection) return;
    const res = await fetch(`/api/binders/${activeBinder.id}/slots`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        collectionEntryId: binderCardSelection,
        page,
          ...
          {binders.length > 0 && (
            <div className="grid gap-3 md:grid-cols-2">
              {binders.map((binder) => (
                <div
                  key={binder.id}
                  className="flex flex-col justify-between rounded border border-slate-800 bg-slate-900/70 p-4"
                >
