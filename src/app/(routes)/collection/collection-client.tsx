            ...
            <div className="flex flex-wrap items-end gap-3">
              ...
              <label className="flex flex-col text-sm text-slate-200">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setBinderForm((s) => ({ ...s, layout: e.target.value as BinderLayout }))
                  }
                  className="rounded border border-slate-700 bg-slate-950/60 p-2 text-white"
                >
                  <option value="grid">Grid (binder pages)</option>
                  <option value="list">Loose cards list</option>
                </select>
              </label>
              ...
              <button
                ...
                className="rounded border border-amber-500/60 px-4 py-2 font-semibold text-amber-100 hover:bg-amber-500/10 disabled:opacity-60"
              >
                {binderSaving ? "Creating..." : "Create binder"}
              </button>
            </div>
            <p className="mt-2 text-xs text-slate-400">
              Kies een grid met pagina's of een losse kaarten lijst. Je kunt deze instellingen later nog aanpassen.
            </p>
          </form>

          {binderLoading && <div className="text-slate-300">Binders laden...</div>}
          {!binderLoading && binders.length === 0 && (
            <div className="rounded border border-slate-800 bg-slate-900/70 p-4 text-slate-300">
              Nog geen binders. Maak er één om kaarten te groeperen.
            </div>
          )}
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {binders.map((binder) => (
                <button
                  ...
                  className={`flex flex-col justify-between rounded border p-4 text-left shadow transition ${
                    activeBinder?.id === binder.id
                      ? "border-amber-500/60 bg-amber-500/10"
                      : "border-slate-800 bg-slate-900/70 hover:border-amber-500/40"
                  }`}
                >
                  <div className="space-y-1">
                    <div className="text-lg font-semibold text-amber-100">{binder.name}</div>
                    <div className="text-sm text-slate-300">
                      {binder.layout === "grid"
                        ? `Grid ${binder.gridRows}x${binder.gridCols}, ${binder.pages} pagina's`
                        : "Losse kaarten (lijst)"}
                    </div>
                    <div className="text-xs text-slate-400">Kaarten in binder: {binder.slots.length}</div>
                    <div className="text-xs text-emerald-200">
                      Waarde: ${getBinderValue(binder).toFixed(2)}
                    </div>
                  </div>
                </button>
              ))}
          ...
          {activeBinder && (
            <div className="space-y-4 rounded-lg border border-slate-800 bg-slate-900/80 p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <div className="text-xl font-semibold text-amber-100">{activeBinder.name}</div>
                  <div className="text-sm text-slate-300">
                    {activeBinder.layout === "grid"
                      ? `Grid ${activeBinder.gridRows}x${activeBinder.gridCols}, ${activeBinder.pages} pagina's`
                      : "Losse kaarten"}
                  </div>
                  <div className="text-xs text-emerald-200">Totale binder waarde: ${getBinderValue(activeBinder).toFixed(2)}</div>
                </div>
                <div className="flex flex-wrap gap-2">
                  <button
                  ...
                  >
                    {binderEditMode ? "Stop editing" : "Edit layout"}
                  </button>
                  <button
                    onClick={() => {
                      setBinderAddMode((v) => !v);
                      setBinderCardSelection(null);
                    }}
                    className="rounded border border-emerald-500/60 px-3 py-2 text-sm font-semibold text-emerald-100 hover:bg-emerald-500/10"
                  >
                    {binderAddMode ? "Close add mode" : "Add cards"}
                  </button>
                </div>
              </div>
                ...
                <div className="rounded border border-slate-800 bg-slate-950/60 p-3">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <button
                      ...
                      disabled={binderSaving}
                      className="rounded border border-emerald-500/60 px-4 py-2 text-sm font-semibold text-emerald-100 hover:bg-emerald-500/10 disabled:opacity-60"
                    >
                      {binderSaving ? "Saving..." : "Save changes"}
                    </button>
                    <span className="text-xs text-slate-400">Aanpassingen gelden direct en houden bestaande kaarten intact.</span>
                  </div>
                </div>
              )}

              {activeBinder.layout === "grid" && activeBinder.pages && (
                <div className="flex items-center justify-between rounded border border-slate-800 bg-slate-950/60 p-3 text-sm text-slate-200">
                  ...
                  {binderAddMode && (
                    <div className="text-xs text-slate-400">Selecteer een kaart en klik een vakje om te plaatsen.</div>
                  )}
                </div>
              )}
              ...
              {activeBinder.layout === "grid" ? (
                <div
                  className="rounded-xl border border-slate-800 bg-gradient-to-br from-slate-950 to-slate-900 p-4 shadow-inner"
                  style={{
                      ...
                      return Array.from({ length: total }).map((_, idx) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex min-h-[140px] flex-col items-center justify-center rounded-lg border bg-slate-950/60 p-3 text-center transition ${
                              selected
                                ? "border-amber-500/80"
                                : slot
                        ...
                        return (
                          ...
                          >
                            <Image
                              ...
                              className="rounded"
                            />
                            <div className="mt-2 font-semibold text-amber-100">{entry.card.name}</div>
                            ...
                            {activeBinder.layout === "grid" ? (
                              ...
                              >
                                {isSelected ? "Geselecteerd" : "Selecteer voor grid"}
                              </button>
                            ) : (
                              <button
